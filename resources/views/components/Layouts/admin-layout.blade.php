    ...
    <nav class="bg-white shadow border-gray-200 fixed top-0 w-screen" style="z-index: 1">
        <div class="flex flex-wrap items-center mx-auto p-4">
            ...
            <a href="/" class="flex items-center space-x-3">
                ...
                <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Admin Management</span>
            </a>
        </div>
    </nav>
    ...
    <aside x-bind:class="sidebarOpened ? 'translate-x-0' : '-translate-x-full'" x-transition class="transition-all fixed top-auto h-full w-64 left-0 border-e pt-16 bg-white">
        <div class="py-3 px-2.5 flex items-center justify-between flex-col h-full">
            <ul class="space-y-2 w-full">
                ...
                <li>
                    @if(request()->routeIS('admin.product-registrations'))
                        <x-button class='!justify-start font-medium' icon='folder' href="{{ route('admin.product-registrations') }}" solid flat full secondary label="Product Registrations" />
                    @else
                        <x-button class='!justify-start font-medium' icon='folder' href="{{ route('admin.product-registrations') }}" flat full secondary label="Product Registrations" /> 
                    @endif
                </li>
