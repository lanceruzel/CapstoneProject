                    ...
                    @if($images)
                        @foreach ($images as $index => $image)
                            <div class="relative group">
                                <img src="{{ is_object($image) && method_exists($image, 'temporaryUrl') ? $image->temporaryUrl() : asset('uploads/reports') . '/' . $image }}" alt="Uploaded image preview" class="w-full h-32 object-cover rounded-lg shadow-md">
