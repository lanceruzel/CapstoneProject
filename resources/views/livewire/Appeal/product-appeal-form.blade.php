<x-modal-card name="productAppealFormModal" title="Report Appeal" align='center' x-cloak x-on:close="$dispatch('clearReportAppealFormModalData')" blurless wire:ignore.self>  
    <div class="flex flex-col gap-2 items-start text-gray-600 overflow-auto">
                        ...
                        @foreach($media as $index => $item)
                            <div class="relative group" wire:key="post-{{ $index }}">
                                @if($this->identifyFileType(is_object($item) && method_exists($item, 'temporaryUrl') ? $item->temporaryUrl() : $item) == 'video')
                                    <video src="{{ is_object($item) && method_exists($item, 'temporaryUrl') ? $item->temporaryUrl() : asset('uploads/reports') . '/' . $item }}" alt="video preview" class="w-full h-32 object-cover rounded-lg shadow-md"></video>
                                @else
                                    <img src="{{ is_object($item) && method_exists($item, 'temporaryUrl') ? $item->temporaryUrl() : asset('uploads/reports') . '/' . $item }}" alt="image preview" class="w-full h-32 object-cover rounded-lg shadow-md">
                                @endif
        ...
        <x-slot name="footer" class="flex justify-end gap-x-4">
            ...
            <x-button wire:loading.attr="disabled" wire:click="store" spinner="store" label="Submit Appeal" />
        </x-slot>
    </div>
</x-modal-card>