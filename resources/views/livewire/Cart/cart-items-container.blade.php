<div class="w-full flex flex-col items-center justify-center">
    @if(count($checkedOutSellers))
        @foreach($checkedOutSellers as $sellerName => $checkedOutSeller)
            <div class="bg-white w-full mt-4 shadow overflow-auto px-3" wire:key="seller-{{ $sellerName }}-container">
                ...
                <table class="w-full">
                    <tbody class="divide-y">
                        @foreach ($checkedOutSeller['products'] as $key => $product)
                            <livewire:Cart.cart-item-container :id="$product->id" wire:key="{{ $sellerName }}-{{ $key }}-cart-{{ $product->id }}-variation-{{ $product->variation }}"/>
                        @endforeach
                    </tbody>
                </table>
            </div>
