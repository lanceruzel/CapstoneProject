<div class="bg-white shadow rounded-lg p-5">
    <div class="flex items-center justify-between">
        <p class="font-semibold text-xl">Ordered lists</p>
    ...
    <div class="w-full pt-5 overflow-auto flex items-center justify-center flex-col">
                ...
                @if(count($ordered) > 0)
                    @foreach ($ordered as $item)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4">{{ date_format($item->order->created_at, "M d, Y") }}</td>
                            <td class="px-6 py-4">
                                <x-button icon="eye" label="View Order" onclick="$openModal('orderViewModal')" wire:click="$dispatch('view-order-info', { id: {{ $item->order_id }}})"  />
                            </td>
                        </tr>
                    @endforeach 
                @endif
        ...
        @if(count($ordered) <= 0)
            <div class="flex flex-col items-center justify-center mt-5">
                <h1 class="text-2xl font-semibold">No ordered products found</h1>
